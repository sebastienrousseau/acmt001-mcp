# Contributing to acmt001-mcp

Thank you for your interest in contributing to acmt001-mcp. This guide covers
the development workflow and standards.

`acmt001-mcp` is the Model Context Protocol (MCP) server of the **acmt001
suite** — alongside the core [`acmt001`](https://github.com/sebastienrousseau/acmt001)
library and the [`acmt001-lsp`](https://github.com/sebastienrousseau/acmt001-lsp)
Language Server. It depends on `acmt001` and exposes its services as agent
...
```bash
# Clone and install
git clone git@github.com:sebastienrousseau/acmt001-mcp.git
cd acmt001-mcp
poetry install
...
> **Note:** `acmt001-mcp` depends on the core `acmt001` library. Until it is
...
5. **Run linters**:
   ```bash
   poetry run ruff check acmt001_mcp/
