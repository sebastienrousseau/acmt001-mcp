...
1. **Preferred:** GitHub's private vulnerability reporting — open a draft
   advisory at <https://github.com/sebastienrousseau/acmt001-mcp/security/advisories/new>.
2. **Email:** `contact@sebastienrousseau.com` with the subject line
   `[acmt001-mcp security]`.
...
- Code under `acmt001_mcp/` shipped to PyPI.
...
- Third-party dependencies (please report upstream — we will track the
  ...
  and the `acmt001` library, which has its own policy.
...
- Records handed to the tools are validated, not executed. Treat account
  data passed through them as PII subject to GDPR/PCI-DSS — it reaches the
  model's context and any transcript kept of it.
- `verify_lei_online` makes an outbound network call. In an environment
  where egress matters, that is the one tool to gate.
- Keep `acmt001-mcp`, `acmt001`, the `mcp` SDK, and the Python interpreter
  patched.
