...
# acmt001-mcp Governance

This document describes how acmt001-mcp is run, how decisions are made,
...
acmt001-mcp is the Model Context Protocol surface for the
[`acmt001`](https://github.com/sebastienrousseau/acmt001) ISO 20022
account management library. It exposes message-type discovery, record
validation, identifier checking, LEI lookup and XML generation as MCP
tools / resources / prompts so AI agents and assistants can drive
account lifecycle flows. Changes are weighed against that scope:
...
Releases follow [`RELEASING.md`](RELEASING.md). acmt001-mcp ships as one
suite with [`acmt001`](https://github.com/sebastienrousseau/acmt001) and
[`acmt001-lsp`](https://github.com/sebastienrousseau/acmt001-lsp) on one
version number. Only maintainers publish to PyPI; release authority
rests with the lead maintainer and is expanding to a second maintainer
as a standing goal.
...
1. Contribute a few reviewed PRs in an area
   ...
   new MCP tool wrappers over `acmt001.services`, examples).
...
acmt001-mcp today has **one** maintainer, which is a real risk for a
